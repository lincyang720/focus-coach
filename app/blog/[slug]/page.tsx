...
export default function ArticlePage({ params }: Props) {
  ...
  return <PageShell>
    ...
    <article className="mx-auto max-w-3xl py-10">
      ...
      <header className="py-8"><p className="font-medium text-primary">{article.keyword}</p><h1 className="mt-3 text-4xl font-semibold leading-tight">{article.title}</h1><p className="mt-5 text-lg leading-8 text-muted-foreground">{article.description}</p><p className="mt-4 text-sm text-muted-foreground">Updated July 20, 2026 · Educational productivity content</p></header>
      ...
      <aside className="mt-12 rounded-lg border bg-background/90 p-6"><h2 className="text-xl font-semibold">Try a 60-second focus reset</h2><p className="mt-3 leading-7 text-muted-foreground">Use the free attention check as a transition into your next task. No account is required.</p><Button className="mt-5" asChild><Link href="/#focus-check">Take the free focus test <ArrowRight className="h-4 w-4" aria-hidden /></Link></Button></aside>
